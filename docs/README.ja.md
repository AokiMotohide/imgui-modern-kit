...
| v2利用者が更新する | [v3移行ガイド](migration-v3.md) |

## 個別moduleを統合する

| module | ガイド | 契約・詳細資料 |
|---|---|---|
| Node Editor | [Node Editor統合](node-editor.ja.md) | [生成API一覧](node-editor-api.json)、[設計review](node-editor-review.md) |
| Workflow・data component | [Workflow component](workflow-components.md) | [Shell component](shell-components.ja.md) |
| Editor Core・Video・CG | [Editor Suite](editor-suite.md) | [Editor API](editor-api.md)、[Timeline編集](timeline-editing.md) |
| Window frame | [Gallery WindowFrameガイド](gallery-window-frame.md) | [WindowFrame API一覧](window-frame-api-inventory.json) |
| Icon | [Icon一覧](icons.md) | [Icon設計audit](design-icon-audit.md) |

## 契約やAPIを確認する

| 確認したいこと | 基準となる文書 |
|---|---|
| ImKitが所有するもの、ホストに残るもの | [Architecture](architecture.md) |
| 必須Dear ImGui revisionと外部package | [Dependencies](dependencies.md) |
| 公開するDear ImGui関数とoverload | [公開API対応表](api-coverage.md)、[API inventory](api-inventory.json) |
| build・操作で確認した範囲と未検証事項 | [Validation](validation.md) |
| widget catalogの対象 | [Widget inventory](widget-inventory.md) |
| design-system tokenとAPI | [Design system](design-system.md)、[Design-system API一覧](design-system-api.json) |
| 製品契約や実装検証の記録 | [開発状況](development-status.md)、[Editor更新記録](editor-refresh.md)、[Editor検証記録](editor-validation.md) |

所有権の基準は`architecture.md`、検証証拠の基準は`validation.md`です。Validationは特定の時点・revisionに対する記録です。過去の合格を後続checkoutの証明として扱わず、native OS入力、accessibility、外部host、実機の受入範囲も個別に確認してください。
