...
The workflow components extend patterns. Image and preview components belong to Editor Core and reuse Canvas/Selection/Splitter. The base `imkit` does not depend on Editor Core. See [workflow-components.md](workflow-components.md) for the public contract.
