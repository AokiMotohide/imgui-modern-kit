...
[日本語](アイコン.md) · [Documentation index](../README.md)
...
Current checks are recorded in [timeline editing](../components/timeline-editing.md). Generation
validates every original/hash/size/atlas. The image below and
[Editor 1.0 acceptance](../archive/editor-validation.md) are the previous baseline.

追加3Dアイコンは`ObjectView::icon`と検索可能なカタログから利用できる。今回の検証結果は
[タイムライン編集](../components/タイムライン編集.md)を参照してください。生成処理は全原画hash・サイズ・
