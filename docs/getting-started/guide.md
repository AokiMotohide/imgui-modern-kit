...
[日本語](利用ガイド.md) · [Documentation index](../README.md)
...
Native ranges use `DragFloatRange2` / `DragIntRange2`; all scalar and vector overloads are in the [API table](../reference/api-coverage.md). Standard wrappers use current ImGuiStyle and do not require a theme argument. Appending DrawList decoration does not submit another item; `IsItem*` continues to refer to the widget. Groups such as unit inputs and segments expose native group status.
