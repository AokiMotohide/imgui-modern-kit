...
[日本語](ノードエディタの作成.md) · [Documentation index](../README.md) · [How ImKit works](../getting-started/how-it-works.md)
...
A `PaletteEntry` is `ne::PaletteEntry{type, title, category, description, user, compatible}`. For richer per-node content (groups, frames, previews), the callback can call `BeginNode`/`EndNode`, `BeginPin`/`EndPin`, and `Preview`; the full reference is [node-editor.md](../components/node-editor.md).
...
- [Node Editor integration (full reference)](../components/node-editor.md)
