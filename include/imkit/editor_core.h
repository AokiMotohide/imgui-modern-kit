...
struct FrameRate { int numerator = 24, denominator = 1; };
...
struct Range { Tick first = 0, last = 0; }; // Half-open.
...
enum class EditKind {
    Move, Duplicate, Remove, TrimStart, TrimEnd, Split, Ripple, Roll, Slip, Slide,
    Select, BoxSelect, LassoSelect, Rename, Reparent, Reorder, Link, Toggle,
    Property, Reset, Keyframe, Handle, Translate, Rotate, Scale, Navigate, Range, Marker
};
...
struct Transaction {
    bool active = false;
    Event draft{};
    bool Begin(StableId id, std::uint64_t revision, EditKind kind, Value original,
               Modifiers modifiers, EventBuffer &events);
...
struct SnapCandidate { Tick tick = 0; SnapKind kind = SnapKind::Frame; int priority = 0; StableId id = 0; };
struct SnapResult { Tick tick = 0; bool snapped = false; SnapCandidate candidate{}; double distancePixels = 0; };
...
struct CanvasView { ImVec2 min{}, max{}; Rect visible{}; bool hovered = false; };
...
enum class Command { PlayPause, Stop, PreviousFrame, NextFrame, SetIn, SetOut, Loop, Split,
                     Delete, Duplicate, SelectAll, Fit, AddKey, PreviousKey, NextKey, Undo, Redo };
enum class ShortcutPreset { CapCut, Premiere, Blender };
struct Binding { Command command{}; ImGuiKeyChord chord = 0; };
...
struct Marker { StableId id = 0; Tick tick = 0; const char *label = ""; };
struct TimeState {
    Tick playhead = 0;
...
double Evaluate(std::span<const Keyframe> sortedKeys, Tick tick, Extrapolation extrapolation = Extrapolation::Constant);
struct CurveQuery { Range time; double minimum = 0, maximum = 0; };
...
struct CurveState { CanvasState canvas{{0, -1}, {100, 100}}; Transaction drag; StableId handle = 0; int side = 0; Point mouseStart{}; };
...
enum class PropertyFlags : unsigned { None = 0, Mixed = 1, Modified = 2, Override = 4, Favorite = 8, Locked = 16, Animated = 32, Keyed = 64 };
struct PropertyView { StableId id = 0; const char *label = "", *category = ""; double value = 0, defaultValue = 0; PropertyFlags flags{}; };
struct PropertyProvider { void *user = nullptr; std::uint64_t revision = 0; int count = 0;
...
struct PropertyState { char search[128]{}; Transaction drag; double draft = 0; };
void PropertyGrid(const char *id, const PropertyProvider &provider, PropertyState &state, EventBuffer &events);
enum class AssetStatus { Ready, Loading, Proxy, Missing, Error };
struct AssetView { StableId id = 0; const char *label = "", *tag = ""; ImTextureRef thumbnail{}; AssetStatus status{}; };
struct AssetProvider { void *user = nullptr; std::uint64_t revision = 0; int count = 0;
    std::span<const AssetView> (*query)(void *, int first, int count, std::string_view search) = nullptr;
};
struct AssetState { char search[128]{}, rename[256]{}; bool grid = true; StableId renaming = 0; };
