...
def main() -> int:
    ...
    if len(source_sizes) != 1:
        raise SystemExit(f"all source frames must have the same dimensions, found {sorted(source_sizes)}")
    source_width, source_height = source_sizes.pop()
    if source_width * args.height != source_height * args.width:
        raise SystemExit(f"source frames must have the same aspect ratio as {args.width}x{args.height}")
    if (source_width, source_height) != (args.width, args.height):
        images = [image.resize((args.width, args.height), Image.Resampling.LANCZOS) for image in images]
    ...
    dither = Image.Dither.FLOYDSTEINBERG if args.dither == "floyd-steinberg" else Image.Dither.NONE
